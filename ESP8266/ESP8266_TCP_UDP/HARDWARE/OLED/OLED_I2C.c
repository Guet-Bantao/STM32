...
void I2C_Configuration(void)
{
	...
	GPIO_SetBits(GPIOB,GPIO_Pin_13|GPIO_Pin_15); 	//PB6,PB7 输出高
}
...
//--------------------------------------------------------------
// Prototype      : void OLED_ShowVar(unsigned char x, unsigned char y, unsigned char ch[], unsigned char TextSize)
// Calls          : 
// Parameters     : x,y -- 起始点坐标(x:0~127, y:0~7); ch[] -- 要显示的数字; TextSize -- 字符大小(1:6*8 ; 2:8*16)
// Description    : 显示codetab.h中的ASCII字符,有6*8和8*16可选择
//--------------------------------------------------------------
...
//x、x1取值为0到8，y、y1取值为0到128
void OLED_Clean(unsigned char x,unsigned char y,unsigned char x1,unsigned char y1,unsigned char fill_Data)
{
	unsigned char m,n;
	for(m=x;m<x1;m++)
	{
