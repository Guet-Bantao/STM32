...
u8 OV7670_Init(void)
{
	...
	GPIO_SetBits(GPIOA,GPIO_Pin_8);
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4;				 // 端口配置 FIFO写使能和读FIFO时钟
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOB,GPIO_Pin_3|GPIO_Pin_4);	

	
	GPIO_InitStructure.GPIO_Pin  = 0xff; //PC0~7 输入 上拉 八位数据引脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
 	GPIO_Init(GPIOC, &GPIO_InitStructure);
	 
	
  GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_6;  //FIFO指针写指针复位
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
 	GPIO_Init(GPIOD, &GPIO_InitStructure);
	GPIO_SetBits(GPIOD,GPIO_Pin_6);
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_14|GPIO_Pin_15;  //fifo读指针复位 fifo输出使能 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
...
//设置图像输出窗口
//对QVGA设置。
void OV7670_Window_Set(u16 sx,u16 sy,u16 width,u16 height)// 行起始点 场起始点 宽度 高度
{
	u16 endx;
	u16 endy;
	u8 temp; 
