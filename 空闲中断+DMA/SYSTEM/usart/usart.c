...
void USART1_IRQHandler(void)                	//串口1中断服务程序
	...
	OSIntEnter();    
#endif
	if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)  //接收中断 注意：USART_IT_IDLE
		{
			USART_ReceiveData(USART1);//读取数据 注意：这句必须要，否则不能够清除中断标志位。
			Usart1_Rec_Cnt = USART_REC_LEN -  DMA_GetCurrDataCounter(DMA1_Channel5);   //算出接收本帧数据长度
			//Usart1_Send(ReceBuff,Usart1_Rec_Cnt);	
			ReceBuff[Usart1_Rec_Cnt] = '\0'; 
