...
{ 	
	...
	GPIO_SetBits(GPIOG,GPIO_Pin_6); 						 //PE.5 输出高
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	    		 //LED1-->PE.5 端口配置, 推挽输出
	GPIO_Init(GPIOB, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
	GPIO_SetBits(GPIOB,GPIO_Pin_15); 						 //PE.5 输出高
	...
	GPIO_Init(GPIOG, &GPIO_InitStructure);	  				 //推挽输出 ，IO口速度为50MHz
	GPIO_SetBits(GPIOG,GPIO_Pin_8); 						 //PE.5 输出高
    ...
    u8 i;
	  ...
	  if(date&0x80)
	    ...
	    MOSI=0;   // byte最高位输出到MOSI
   	  date<<=1;             // 低一位移位到最高位
   	  SCLK=1; 
	  if(READMISO)               // 拉高SCK，nRF24L01从MOSI读入1位数据，同时从MISO输出1位数据
...
void NRFSetRXMode(u8 *Rx_addr)
{
	...
	NRFWriteReg(W_REGISTER+EN_AA,0x3f);     //3f          // 使能接收通道0自动应答
  	NRFWriteReg(W_REGISTER+EN_RXADDR,0x3f);      //3f     // 使能接收通道0
...
{//发送模式
	...
	/******下面有关寄 存器配置**************/
  	NRFWriteReg(W_REGISTER+EN_AA,0x3f);       // 使能接收通道自动应答
  	NRFWriteReg(W_REGISTER+EN_RXADDR,0x3f);   // 使能接收通道
    NRFWriteReg(W_REGISTER+SETUP_RETR,0x1a);  // 自动重发延时等待250us+86us，自动重发10次
	NRFWriteReg(W_REGISTER+RF_CH,0x40);         // 选择射频通道0x40
  	NRFWriteReg(W_REGISTER+RF_SETUP,0x0f);    // 数据传输率1Mbps，发射功率0dBm，低噪声放大器增益
