...
void OSTimeDly(unsigned int ticks)
{
	...
	if(ticks> 0)
	{
		...
		if(TCB_Task[OS_PrioCur].OSTCBStatPend==OS_STAT_MUTEX_DLY)//如果是等待互斥资源
		{
			p_TCBHightRdy->DLy=ticks;
			p_TCBHightRdy=&TCB_Task[OS_PrioCur];
			OS_EXIT_CRITICAL(); 
			OSCtxSw();
			return ;
		}
...
void OS_MutexPend(ECB  *pevent)
{
	...
	{
		pevent ->Prio=OS_PrioCur;			//记录占用资源任务的优先级(优先级可被提升)
		pevent ->Cnt=OS_PrioCur;			//记录最原始的占用资源任务的优先级
		OS_EXIT_CRITICAL();
		return ;
	}
	else if(pevent->Prio < OS_PrioCur)		//如果占用资源的任务的优先级比此任务高(优先级数值小)
	{
		pevent->OSEventTbl|=0x01<<OS_PrioCur;//添加到等待表
		OSDelPrioRdy(OS_PrioCur);			//从就绪表中删除
		OS_EXIT_CRITICAL();
	...
	else									//如果占用资源的任务的优先级比此任务低
	{
		pevent->OSEventTbl|=0x01<<OS_PrioCur;//添加等待资源的任务到等待表
//		OSDelPrioRdy(pevent ->Prio);		//从就绪表中删除占用资源的任务(上一次高优先级任务)
		prio=pevent ->Prio;					//记录上一次申请资源的任务
		pevent ->Prio=OS_PrioCur;			//提升占用资源任务的优先级		
		while(pevent ->Prio!=OS_MAX_Task)	//如果资源没被释放
		{
			if(OSRdyTbl&(0x01<<(pevent ->Cnt)))//如果占用资源的任务处于就绪状态
			{
				if(prio!=pevent ->Prio)		//有前高优先级任务在申请资源
					OSDelPrioRdy(prio);		//从就绪表中删除上一次申请资源的任务
				p_TCBHightRdy=&TCB_Task[pevent ->Cnt];//间接提升资源任务的优先级为此等待任务的优先级
...
void OS_MutexPost(ECB  *pevent)
{
	...
	OS_PrioCur=pevent->Cnt;					//OS_PrioCur更新为恢复被提升的优先级
	for(OS_ECB_Prio=0;						//找出等待表中优先级最高的任务
			(OS_ECB_Prio<OS_MAX_Event)&&(!((pevent->OSEventTbl)&(0x01<<OS_ECB_Prio)));
			OS_ECB_Prio++);
	pevent->OSEventTbl&=~(0x01<<OS_ECB_Prio);//从等待表中删除
	OSSetPrioRdy(OS_ECB_Prio);				//添加等待表中优先级最高的任务到就绪表中
	OSSetPrioRdy(pevent->Cnt);				//恢复占用资源的任务到就绪表中(提升优先级时会把低优先级的申请资源的任务删除掉)
	OS_EXIT_CRITICAL();
	OS_Sched();
}
